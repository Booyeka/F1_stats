...
driver2_data = get_brake_segments(driver_abr="PIA", race_id=1)
driver2_brake_segments = driver2_data[0]
driver2_lap_distances = driver2_data[1]
driver2_tire_compounds = driver2_data[2]


normalized_brake_segments1 = []
normalized_brake_segments2 = []
...
for lap, start_dist, end_dist in driver2_brake_segments:
    lap_start, lap_end = driver2_lap_distances[lap]
    lap_length = lap_end - lap_start

    norm_start = (start_dist - lap_start) / lap_length * standard_lap_length1
    norm_end = (end_dist - lap_start) / lap_length * standard_lap_length1

    normalized_brake_segments2.append((lap, norm_start, norm_end))


brake_segments_driver1 = normalized_brake_segments1
brake_segments_driver2 = normalized_brake_segments2
...
# Unpack all lap numbers and distances to compute axis limits
lap_numbers2 = [lap for lap, _, _ in brake_segments_driver2]
start_distances2 = [start for _, start, _ in brake_segments_driver2]
end_distances2 = [end for _, _, end in brake_segments_driver2]

min_dist = min(min(start_distances1), min(start_distances2))
max_dist = min(max(end_distances1), max(end_distances2))
min_lap = min(min(lap_numbers1), min(lap_numbers2))
max_lap = max(max(lap_numbers1), max(lap_numbers2))
...
# for i, compound in enumerate(driver2_tire_compounds):
#     color = compound_colors.get(compound, 'white')
#     ax.axhspan(i + 0.5, i + 1.5, color=color, alpha=0.2)
#     # ax.axvspan(i + 0.5, i + 1.5, color=color, alpha=0.2)
...
for lap_number, start_dist, end_dist in brake_segments_driver1:
    y_base = lap_number * 2
    # (start_dist, y_base + driver_index)

    ax.add_patch(
        patches.Rectangle(
            (start_dist, y_base + 0 ),  # (x, y): distance and vertical center of lap row
...
for lap_number, start_dist, end_dist in brake_segments_driver2:
    y_base = lap_number * 2
    # (start_dist, y_base + driver_index)
    ax.add_patch(
        patches.Rectangle(
            (start_dist, y_base + 1 ),  # (x, y): distance and vertical center of lap row
            # (lap_number - 0.4, start_dist),  # (x, y): distance and vertical center of lap row
            end_dist - start_dist,          # width (brake zone length)
            # 0.8,
            0.8,                            # height (how thick the row bar is)
...
ax.set_ylim(min_lap - 1, max_lap * 2)
